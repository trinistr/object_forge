...
module ObjectForge
  ...
  #   mold = ->(forged:, attributes:, **) do
  #     Enumerator.new(attributes.size) do |y|
  #       attributes.each_pair { y << forged.dump(_1 => _2) }
  #     end
  #   end
  #   mold.call(forged: JSON, attributes: {a:1, b:2}).to_a
  #     # => ["{\"a\":1}", "{\"b\":2}"]
  #   mold.call(forged: YAML, attributes: {a:1, b:2}).to_a
  #     # => ["---\n:a: 1\n", "---\n:b: 2\n"]
  # @example Abstract factory pattern (kind of)
  #   class FurnitureFactory
  #     def call(attributes:, **)
  #       concrete_factory = concrete_factory(attributes[:style])
  ...
  #     end
  #   end
  #   FurnitureFactory.new.call(forged: nil, attributes: {
  #     style: :hitech, pieces: [:chair, :table], color: { chair: :black, table: :white }
  #   })
